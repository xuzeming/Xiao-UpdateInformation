...
public class RedisUtil {
	...
	private static String CACHE_PREFIX; // key的前缀
	private static boolean CACHE_CLOSED; // 记录redis服务器是否关闭
	private static final Logger LOG = LoggerFactory.getLogger(RedisUtil.class); // 日志对象
	...
	public static Set<String> keys(String patternKey) {
		...
		try {
			if (isClose() || isEmpty(patternKey)) {
				return Collections.emptySet();// 空的set对象 null new Set();
			}
